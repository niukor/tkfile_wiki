...
class Wiki
{
    ...
    protected $_default_page_data = array(
        ...
        'description' => 'Java,全栈工程师,后端开发,前端开发,数据库,Spring框架,Spring Boot,RESTful API,MVC模式,Web开发,HTML,CSS,JavaScript,jQuery,Angular,React,数据结构,网络编程,安全性,软件测试.',
        'tags' => array('架构设计(Architecture Design)','微服务架构(Microservices Architecture)','高可用性(High Availability)','扩展性(Scalability)','性能优化(Performance Optimization)','安全性(Security)','云计算(Cloud Computing)','分布式系统(Distributed Systems)','容器化(Containerization)','事件驱动架构(Event-Driven Architecture)'),
        'page' => ''
    );
